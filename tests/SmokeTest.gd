...
func _run() -> void:
	...
	var assign_billy_home := GameManager.assign_survivor_to_building(1, 1)
	_assert_true(bool(assign_billy_home.get("ok", false)), "can assign crew survivor to controlled base")
	var assign_billy_workshop := GameManager.assign_survivor_to_building(2, 1)
	...
	var npc_assign := GameManager.assign_survivor_to_building(2, new_survivor_id)
	_assert_true(not bool(npc_assign.get("ok", false)), "NPC residents cannot be manually assigned to buildings")
	...
	var saved_party_start := GameManager.scavenge_party("Pharmacy", [1, new_survivor_id])
	_assert_true(bool(saved_party_start.get("ok", false)), "can start a party expedition that will be saved")
	_assert_true(SaveManager.save_game(GameManager.event_log), "save file can store active party expedition")
	ActivityManager.active_jobs.clear()
	SurvivorManager.assign_task(1, "Rest")
	SurvivorManager.assign_task(new_survivor_id, "Rest")
	var loaded_party_save := SaveManager.load_game()
	_assert_true(not loaded_party_save.is_empty(), "save file with active party expedition can be loaded")
	_assert_eq(String(ActivityManager.get_job(1).get("location", "")), "Pharmacy", "lead party job restores expedition location")
	_assert_eq(Array(ActivityManager.get_job(new_survivor_id).get("party_ids", [])).size(), 2, "party member job restores party ids")
	ActivityManager.call("_complete_job", 1)
	_assert_eq(String(SurvivorManager.survivors[0].get("assigned_task", "")), "Rest", "loaded party lead returns to rest after completion")
	_assert_eq(String(SurvivorManager.survivors[1].get("assigned_task", "")), "Rest", "loaded party member returns to rest after completion")
